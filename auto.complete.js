...
$.autocomplete = function(input, options) {
    ...
    function selectItem(li) {
        ...
        if(options.mode == "multiple") {
            old_value = $input.val();
            if(old_value.lastIndexOf(options.separator) >= 1) {
                comma_pos = old_value.lastIndexOf(options.separator);
                value = old_value.substr(0,comma_pos+1);
                new_value = value + ' ' + v +options.separator;
            } else {
                new_value = v + options.separator;
            }
        }
    ...
    function makeUrl(q) {
        if(options.mode == "multiple") {
            if(q.lastIndexOf(options.separator) >= 1) {
                comma_pos = q.lastIndexOf(options.separator);
                q = q.substr(comma_pos+1);
            } 
        }
